                    ...
                    {({ isSubmitting , status }) => (
                        <Form className="bg-white p-8 rounded-lg shadow-lg w-full max-w-sm">
                            <div className="flex items-center justify-center flex-col mb-3">
                                <ImgContainer img="/Remove.png" imgAlt="logo" className="fill" smWidth="w-20" mdWidth="w-25" height="h-25"/>
                                <h2 className="text-xl font-semibold text-center">intranet</h2>
                            </div>
                            <div className="mb-5 relative">
                                <Field type="email" name="email" placeholder="Email" autoFocus aria-label="Email" className="w-full p-2 pl-10 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                                <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none text-lg">
                                    <TfiEmail />
                                </div>
                            </div>
                            <ErrorMessage name="email" component="div" className="text-red-600 text-sm mt-1 mb-3"/>
                            <div className="mb-5 relative">
                                <Field type={showPassword ? "text" : "password"} name="password" placeholder="Password" aria-label="Password" className="w-full p-2 pl-10 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                                <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none text-lg">
                                    <PiLockKey />
                                </div>
                            ...
                            <ErrorMessage name="password" component="div" className="text-red-600 text-sm mt-1 mb-3"/>
                            <button type="submit" disabled={isSubmitting} className="w-full bg-[var(--color-primary)] text-white py-2 rounded font-bold hover:bg-sky-800 disabled:opacity-50" >
